...
Either player SHALL be able to press Start when the room has 2 players and both have teams assigned. The server SHALL broadcast a 3-2-1-GO countdown to both clients before the game begins.

#### Scenario: Player presses Start with room ready
...
#### Scenario: Player presses Start without full room
- **WHEN** a player presses Start but the room has only 1 player
...
After a game ends (completed or abandoned), players remaining in the room SHALL be able to start a new game without leaving the room.
...
Rooms SHALL transition through defined statuses: `empty` → `waiting` → `ready` → `countdown` → `playing` → `gameover`. After game over, rooms return to `ready` (play again) or `waiting`/`empty` (players leave).

#### Scenario: Full room lifecycle
- **WHEN** a room goes through a complete game cycle
